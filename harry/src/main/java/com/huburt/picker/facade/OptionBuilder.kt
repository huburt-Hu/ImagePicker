...
import com.huburt.picker.ui.ShadowActivity
...
class OptionBuilder {
    ...
    fun gridExpectedSize(size: Float): OptionBuilder {
        PickOption.gridExpectedSize = size
        return this
    }

    fun spanCount(count: Int): OptionBuilder {
        if (count < 1) {
            throw IllegalArgumentException("spanCount cannot be less than 1")
        }
        PickOption.spanCount = count
        return this
    }
    ...
    fun forResult(resultListener: OnPickImageResultListener) {
        val activity = Harry.getActivity() ?: return
        Harry.resultListener = resultListener
        val intent = Intent(activity, ShadowActivity::class.java)
        activity.startActivity(intent)
    }

    fun openCamera(resultListener: OnPickImageResultListener) {
        PickOption.openCamera = true
