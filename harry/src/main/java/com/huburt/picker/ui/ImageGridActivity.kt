...
import com.huburt.picker.ui.adapter.ImageFolderAdapter
import com.huburt.picker.ui.adapter.ImageRecyclerAdapter
...
class ImageGridActivity : BaseActivity(), View.OnClickListener, ImageDataSource.OnImagesLoadedListener, ImageRecyclerAdapter.OnImageItemClickListener {
    ...
    private fun initView() {
        ...
        val spanCount = if (PickOption.gridExpectedSize > 0) {
            val screenWidth = resources.displayMetrics.widthPixels
            val expected = Math.round(screenWidth / PickOption.gridExpectedSize)
            if (expected < 1) 1 else expected
        } else PickOption.spanCount

        recycler.layoutManager = GridLayoutManager(this, spanCount)
        recycler.addItemDecoration(GridSpacingItemDecoration(spanCount,
    ...
    override fun onImageItemClick(imageItem: ImageItem, position: Int) {
        if (!PickOption.singleMode) {
            ...
            if (images.size > INTENT_MAX) {//数据量过大
                ...
                p = position - s
                Log.i("hubert", "start:$s , end:$e , position:$p")
//            images = ArrayList()
//            for (i in s until e) {
...
//            }
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (requestCode == REQUEST_CAMERA) {//相机返回
            if (resultCode == Activity.RESULT_OK) {
                Log.i(Harry.javaClass.simpleName, takeImageFile.absolutePath)
                //广播通知新增图片
    ...
    private fun setResult() {
        val result = Intent()
        result.putExtra(Harry.EXTRA_IMAGE_ITEMS, PickOption.selectedCollection.getList())
        setResult(Harry.RESULT_OK, result)
        finish()
    }
